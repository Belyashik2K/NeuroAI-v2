...
@router.message(any_state, F.text == LazyProxy("buttons-about"))
...
async def about_service(call: types.CallbackQuery, user: User, state: FSMContext, i18n: I18nContext):
    await state.clear()
    await call.message.edit_text(i18n.messages.about(**await database.get_neuro_statuses()),
                                 reply_markup=inline.about())
